...
class Field {
  ...
  setHat() {
    this.hatPos = this.setPosition(this.start);
  ...
  addHoles() {
    ...
    for (let i = 1; i <= numHoles; i++) {
      let holePos = {};
      do {
        holePos = this.setPosition(this.hatPos);
      } while (
        holePos.x === this.locationX && holePos.y === this.locationY
      );
  ...
  static generateField(fieldH, fieldW, percentage = 0.1) {
    const field = new Array(fieldH).fill(0).map(row => new Array(fieldW));
...
const myField = new Field(Field.generateField(5, 5, 0.2), true);
